...
function serverRedact(val: string): string {
  return val
...
function redactRecursive(obj: any, depth = 0) {
  if (depth > 6 || obj == null) return;
  if (typeof obj === 'string') return serverRedact(obj);
  if (Array.isArray(obj)) {
    for (let i = 0; i < obj.length; i++) {
      const v = obj[i];
      const nv = redactRecursive(v, depth + 1);
      if (nv !== undefined) obj[i] = nv;
    }
    return;
  }
...
app.use((req: express.Request, _res: express.Response, next: express.NextFunction) => {
  try { if (req.body) redactRecursive(req.body); } catch {/* ignore */}
  next();
});
...
const rateWindows: Record<string, { count: number; reset: number }> = {};
