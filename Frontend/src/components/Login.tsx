...
export default function Login({ onLogin }: { onLogin: (user: AuthUser) => void }) {
  ...
  const validate = () => {
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) return t('validation.email') as string;
    if (mode === 'signup' && name.trim().length < 2) return t('validation.name') as string;
    if (password.length < 6) return t('validation.password') as string;
    return '';
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setStatus({ type: 'success', msg: mode === 'signup' ? t('auth.accountCreated') as string : t('auth.signedIn') as string });
    ...
    <div className="max-w-md mx-auto px-4 sm:px-0">
      <div className="card card-section">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-2xl font-bold text-gray-900">{mode === 'signin' ? t('auth.welcomeBack') : t('auth.createAccountHeadline')}</h2>
            <p className="text-gray-600">{mode === 'signin' ? t('auth.signInCta') : t('auth.startUsing')}</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex items-center justify-between">
            ...
            <button type="button" onClick={() => setMode(mode === 'signin' ? 'signup' : 'signin')} className="text-blue-600 text-sm hover:text-blue-700">
              {mode === 'signin' ? t('auth.createAccountLink') : t('auth.haveAccount')}
            </button>
          </div>
